...
export default function Footer({ onHomePage = true }) {
    ...
    <footer className="border-t border-amber-500/10 bg-ink-950/60 px-6 py-10 backdrop-blur-[3px]">
      <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 font-sans text-xs text-parchment-400 sm:flex-row">
        ...
        <div className="flex gap-6">
          <a href={`${prefix}#the-idea`} className="hover:text-amber-300">The Idea</a>
          <a href={`${prefix}#gatherings`} className="hover:text-amber-300">Gatherings</a>
          <a href={`${prefix}#history`} className="hover:text-amber-300">History</a>
          <a href={`${prefix}#gallery`} className="hover:text-amber-300">Gallery</a>
