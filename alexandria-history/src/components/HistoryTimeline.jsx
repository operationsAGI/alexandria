...
function EventRow({ ev }) {
  ...
  const links = []
  if (ev.rsvp) links.push({ href: ev.rsvp, label: 'Event Link' })
  ...
  return (
    <li ref={rowRef} className="tl-reveal relative pb-12 pl-9 last:pb-0">
      ...
      {links.length > 0 ? (
        ...
        <p className="font-sans text-sm text-parchment-400">{ev.note || 'Details on Luma'}</p>
      )}
    </li>
  )
}
          ...
          {years.map((year) => (
            <div key={year} className="mb-2">
              ...
              <ul className="list-none">
                {eventsByYear[year].slice().reverse().map((ev) => (
